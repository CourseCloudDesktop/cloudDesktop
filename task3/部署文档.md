...
- 根据提示设置新实例的相关信息
  ...
  - 设置flavor，即给这个新的实例分配多少的计算资源，一共有五种档次，按个人硬件水平选择。在这里选择的是m1.small，因为选择太低的配置可能会导致实例创建失败。
	![](https://github.com/CourseCloudDesktop/cloudDesktop/raw/wcl-develop/task3/screenshot/5.PNG)
  - 设置网络，不要选择外部网络，而是选择虚拟机所在的子网。点+，选择合适的网络。这里选择的是demo_net
...
- 点击实例的名字，然后点击控制台，经过漫长等待应该可以见到熟悉的Ubuntu安装画面，在这里我们就可以跟像在本地操作Ubuntu一样操作这个云主机了
...
### 2.2 实例运行起来很卡
- 不要装对配置要求很高的系统镜像
- 给虚拟机分配更大的运行内存更多的CPU，至于硬盘的话，如果在暗转Ubuntu Server系统时用的是手动分配内存大小，那么硬盘不能再分配更多的空间了，只能重装的时候再分配更大的了。
- 升级电脑的硬件

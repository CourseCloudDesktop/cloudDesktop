...
<div align=center> <img src="https://github.com/CourseCloudDesktop/cloudDesktop/blob/yxf-develop/task6/image/3.png" width="60%" height="60%" /></div>
    ...
    通过使用过滤器来选择最优的计算节点：
介绍 nova-scheduler 的调度机制和实现方法：即解决如何选择在哪个计算节点上启动 instance 的问题。通过介绍Filter scheduler过滤器，来讲述nova-scheduler
Filter scheduler 是 nova-scheduler 默认的调度器，调度过程分为两步： 通过过滤器（filter）选择满足条件的计算节点（运行 nova-compute）

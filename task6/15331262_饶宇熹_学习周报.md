...
[3 Nova-scheduler介绍](#Nova-scheduler介绍)
* 3.1 placement中的过滤机制
* 3.2 placement中的称重机制

[4 Nova-scheduler源码分析](#Nova-scheduler源码分析)
...
[附录 任务中的工作量、学习的路线规划以及心得](#心得)



<h1 id="OpenStack概况">1 OpenStack概况</h1>
...
<h1 id="Nova-scheduler介绍">3 Nova-scheduler介绍</h1>
...
<h1 id="Nova-scheduler">4 Nova-scheduler</h1>


对Nova-scheduler关键代码及其意义进行分析

`/nova/scheduler/driver.py`: 文件中最重要的就是 Scheduler 类，是所有调度器实现都要继承的基类，包含了调度器必须要实现的所有接口。</br>
`/nova/scheduler/manager.py`: 主要实现了 SchedulerManager 类，定义了 Host 的管理操作函数，如：删除 Host 中的 Instance — delete_instance_info</br>
`/nova/scheduler/host_manager.py`: 有两个类的实现，都是描述了跟调度器相关的 Host 的操作实现，类 HostState 维护了一份最新的 Host 资源数据。类 HostManager 描述了调度器相关的操作函数， EG._choose_host_filters/get_filtered_hosts/get_weighed_hosts</br>
`/nova/scheduler/chance.py`: 只有 ChanceScheduler 类(随机调度器)，继承自 Scheduler 类，实现随机选取 Host Node 的调度器</br>
`/nova/scheduler/client`: 客户端调用程序的入口</br>
`/nova/scheduler/filter_scheduler.py`: 只有 FilterScheduler 类(过滤调度器)，继承自 Scheduler 类，实现了根据指定的过滤条件来选取 Host Node 的调度器 </br>
`/nova/scheduler/filters` 和 `/nova/scheduler/weights`: 这两个目录下的内容分别对应 filter 和 weighting 的实现 。</br>
...
<h1 id="心得">5 任务中的工作量、学习的路线规划以及心得</h1>

## 任务中的工作量

学习任务
* 系统学习OpenStack架构，对七个核心组件和四种通信方式有基本的了解，为Nova的学习做铺垫
* 系统学习Nova架构，并对Nova进行拆分，学习主要组件的基本功能和Nova的运作方式
* 学习Nova-scheduler的过滤机制
* 尝试模仿写了一个小小的filter
* 阅读并简要分析Nova-scheduler的部分源码

小组汇报任务
* 负责PPT第一部分“NOVA主要组件介绍”的制作
* 负责汇报第一部分，内容包括OpenStack概况介绍、Nova总体架构介绍、Nova主要组件和组件间通信关系的介绍

## 学习的路线规划

学习Nova的时候，我发现其实工作量是非常巨大的。一开始我们本来计划学习Nova-compute，但是学习Nova-compute预备知识已经花了好多气力，想要在两周时间内弄明白Nova-compute，时间并不很充分。
我们想，与其随随便便敷衍，不如实实在在研究清楚一个小问题。于是我们改为学习nova-scheduler，弄清楚Nova对虚拟机的调度机制。

针对nova-scheduler，我们大致的学习路线：</br>
1.花了两三天浏览技术博客，看看图，收集有用的文档</br>
2.做一点学习nova-scheduler准备工作，学习预备知识</br>
3.读源码，每天读一点，边读边写周报，用文字写一下有助于理解</br>
4.花了两天整理，做PPT</br>
...
经过一段时间OpenStack学习，从最初的挣扎到现在的部署成功并且能够针对某一组件进行深入学习，感觉很有收获。

之前动手部署的过程中，虽然过程很繁琐，也遇到了许多困难，但其实是“因祸得福”。反复的安装反倒使我得以直观了解OpenStack的各个组件。重装的过程中还经历了反反复复删除Cinder、Neutron、Horizon、Glance的折磨，当初也不知道这些是什么，经过两个多月的学习和这次最终考核的系统学习之后，终于有所理解，原来这些都是OpenStack里的核心服务。

之前部署的时候，出错最多的地方就是Nova的安装，耗时非常长。一旦Nova的安装完成了，部署成功就胜利在望了。一开始不了解是怎么回事，也不知道Nova是什么，只是知道有Nova这个东西，并且潜意识觉得Nova非常难搞。而这次最终考核的选题我们却选了Nova。实训的开始，我就是从攻克Nova安装的难关开始的，现在兜兜转转又回到了Nova组件的学习，感觉有点小小的圆满。

在最终考核的准备过程中，我对很多知识的理解都来源于OpenStack官方文档。多结合OpenStack的官方文档来进行模块学习。虽然官方文档全是英文，但是内容很多很全，挑着自己需要的看，帮助很大。当然，更要注重实践与理论结合。这也是我们这次实训一直在做的。
虽然实训接近尾声了，但是对OpenStack的学习和探索还远远没有结束。
